...
} from './providers/jwt-auth-provider.config';
...
export class AuthInterceptorService implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    if (!this.configService.httpInterceptor?.allowedList) {
    ...
    return this.findMatchingRoute(req, this.configService.httpInterceptor).pipe(
      concatMap((route) =>
        iif(
          ...
          of(route).pipe(
            map((z: any) => {
              return z?.tokenOptions as SilentTokenOptions;
            }),
            concatMap<SilentTokenOptions, Observable<TokenInfo | null>>((options) => {
